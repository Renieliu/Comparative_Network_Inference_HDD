...
for (subsample in subsample_sizes) {
  ...
  for (i in 1:num_simulations) {
    ...
    data <- as.matrix(dat)
    #fit <- huge(data, method = "glasso", nlambda = 30) # Fit graphical models using a sequence of lambdas
    fit <- huge(data, method = "glasso", nlambda = 10)
    
    select_ebic <- huge.select(fit, criterion = "ebic") # Select the best model using EBIC (default)
    lambda_best <- select_ebic$opt.lambda # Get the selected lambda and graph
    lasso <- select_ebic$refit
  ...
  write.csv(count_data, paste0("Synthetic_count_data_", subsample,"_LASSO_",seed,".csv"))
  write.csv(metric_data, paste0("Synthetic_metric_data_", subsample,"_LASSO_",seed,".csv"))
}